...
func (t *MyT) AssertFiles(expectedFileInfo []fileinfoTest, v *Fs, format string, args ...interface{}) {
	...
	v.Walk("/", func(path string, fi os.FileInfo) error {
		...
		if expectedFI.modTime != ignoreTime {
			t.AssertEqual(expectedFI.modTime, n.modTime, "%v modeTime doesnt match %v", str, count)
		}
		...
		t.AssertEqual(expectedFI.symlinkPath, n.symlinkPath, "%v symlinkPath doesnt match %v", str, count)
